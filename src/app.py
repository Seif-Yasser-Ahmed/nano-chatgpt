...
model = GPT.load_custom_checkpoint(
    "out_gpt2/10B/finetuned/ckpt_21999.pt", device=device, visualize_attention=True)
model.eval()

# Helper function to strip Gradio 6's content blocks back to raw strings
...
def chat_and_visualize(user_input, prior_history):
    ...
    # THE FIX 2: Restored max_new_tokens to 256 to match the CLI script
    with torch.no_grad():
        generated_idx = model.generate(
            xgen, max_new_tokens=256, temperature=0.7, top_k=40)
